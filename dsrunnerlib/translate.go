...
func (x *translator) translateTransform(trunk string, id string) ([]*df.Step, error) {
	...
	case graphx.URNCombinePerKey:
		...
		if len(t.Subtransforms) != 2 {
			return nil, errors.New(fmt.Sprintf("invalid CombinePerKey, expected 2 subtransforms but got %d in %v", len(t.Subtransforms), t))
		}
		steps, err := x.translateTransforms(fmt.Sprintf("%v%v/", trunk, path.Base(t.UniqueName)), t.Subtransforms)
		if err != nil {
			return nil, errors.New(fmt.Sprintf("invalid CombinePerKey, couldn't extract GBK from %v", t))
		}
		var payload pipepb.CombinePayload
		if err := proto.Unmarshal(t.Spec.Payload, &payload); err != nil {
			return nil, errors.New(fmt.Sprintf("invalid Combine payload for %v", t))
		}

		c, err := x.coders.Coder(payload.AccumulatorCoderId)
		if err != nil {
			return nil, errors.New(fmt.Sprintf("invalid Combine payload , missing Accumulator Coder %v", t))
		}
		enc, err := graphx.EncodeCoderRef(c)
		if err != nil {
			return nil, errors.New(fmt.Sprintf("invalid Combine payload, couldn't encode Accumulator Coder %v", t))
		}
	...
	case pubsub_v1.PubSubPayloadURN:
		...
		if err := proto.Unmarshal(t.Spec.Payload, &msg); err != nil {
			return nil, errors.New("bad pubsub payload")
		}
		...
		default:
			return nil, errors.New(fmt.Sprintf("bad pubsub op: %v", msg.Op))
		}
	...
	default:
		...
		return nil, errors.New(fmt.Sprintf("unexpected primitive urn: %v", t))
	}
}
...
func (x *translator) wrapCoder(pcol *pipepb.PCollection, c *coder.Coder) *graphx.CoderRef {
	...
	if err != nil {
		panic(errors.New(fmt.Sprintf("failed to decode window coder %v for windowing strategy %v", ws.WindowCoderId, pcol.WindowingStrategyId)))
	}
	ret, err := graphx.EncodeCoderRef(coder.NewW(c, wc))
	if err != nil {
		panic(errors.New(fmt.Sprintf("failed to wrap coder %v for windowing strategy %v", c, pcol.WindowingStrategyId)))
	}
	return ret
}
